...
from dispenser_controller import DispenserController  # 更新导入
...
class DispenserControllerWrapper(QObject):
    """分药机控制器，运行在单独的线程中"""
    # 信号定义
    ...
    plate_presence_checked_signal = Signal(bool)  # 药盘存在检查信号
    ...
    def initialize_hardware(self):
        ...
        try:
            # 初始化分药机
            print("[初始化] 连接分药机...")
            self.dispenser = DispenserController(self.dispenser_port)  # 使用新的控制器
            ...
            # 初始化分药机
            init_result = self.dispenser.initialize_dispenser()  # 使用新的方法名
            if init_result != 0:
    ...
    def cleanup_hardware(self):
        ...
        try:    
            if self.dispenser:
                self.dispenser.close_tray()  # 确保药盘关闭，使用新的方法名
                self.dispenser.close()  # 使用新的关闭方法
    ...
    def prepare_for_rfid_detection(self):
        ...
        try:
            ...
            print("[分药机] 正在打开药盘...")
            open_result = self.dispenser.open_tray()  # 使用新的方法名
    ...
    def dispense_next_medicine(self):
        ...
        try:
            ...
            try:
                # 设置转盘电机速度
                speed_result = self.dispenser.set_turnMotor_speed(0.8)
                if speed_result != 0: 
                    print(f"[警告] 设置转盘电机速度失败，错误码: {speed_result}")
                else:
                    print(f"[设置] 转盘电机速度设置为: 0.6")
                
                # 设置舵机角度
                angle_result = self.dispenser.set_servo_angle(0.5)
                if angle_result != 0:
                    print(f"[警告] 设置舵机角度失败，错误码: {angle_result}")
                else:
                    print(f"[设置] 舵机角度设置为: 0.5")
                    
            except Exception as e:
                print(f"[警告] 设置分药机参数异常: {str(e)}")
                # 继续执行分药，不因参数设置失败而中断
    ...
    def close_plate(self):
        ...
        try:
            if not self.dispenser:
                ...
                print("[分药机] 正在关闭药盘...")
                close_result = self.dispenser.close_tray()  # 使用新的方法名
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # 初始化分药机控制器
        self.controller = DispenserControllerWrapper()  # 使用包装器类
    ...
    def connection(self):
        ...
        self.ui.finish_dispensing_button.clicked.connect(self.safe_finish_dispensing)
        self.ui.refresh_database_button.clicked.connect(self.refresh_prescription_database)
    ...
    def move_to_put_pan_in_page(self):
        ...
        self.ui.finish_dispensing_button.hide()  # Hide the finish button initially
    ...
    def update_error_msg(self, error_msg):
        ...
        if "RFID读取失败" in error_msg:
            self.ui.get_prescription_msg.setText("RFID读取失败，放入药盘后点击按钮重新读取")
            self.ui.get_prescription_msg.show()
        ...
        if "生成分药矩阵失败" in error_msg:
            self.ui.get_prescription_msg.setText("处方读取失败，放入药盘后点击按钮重新读取")
            self.ui.get_prescription_msg.show()
            self.ui.error_mark_2.show()
            self.ui.check_mark_2.hide()
            self.ui.refresh_rfid_button.show()  # 显示刷新按钮
    ...
    def update_to_pills_dispensing_list_label(self):
        ...
        self.ui.guide_msg_2.setText("读取处方中...")
    ...
    def prepare_for_dispensing(self):
        ...
        if not self.controller.hardware_initialized:
            self.ui.guide_msg_2.setText("初始化分药机中...")
            self.ui.pan_img.hide()
            self.ui.green_arrow.hide()
            from PySide6.QtCore import QMetaObject, Qt
    ...
    # 处理安全结束分药流程
    @Slot(bool)
    def on_plate_presence_checked(self, plate_present):
        """处理药盘存在检测结果"""
        self.plate_on_tray = plate_present
        self.checking_plate_presence = False
        
        if plate_present:
            # 药盘还在托盘上，显示警告信息
            self.show_plate_removal_warning()
        else:
            # 药盘不在托盘上，可以安全结束
            self.proceed_with_finish_dispensing()

    def show_plate_removal_warning(self):
        """显示药盘移除警告"""
        # 可以在UI上显示警告信息
        if hasattr(self.ui, 'finish_dispensing_button'):
            # 暂时禁用按钮并更改文本
            self.ui.finish_dispensing_button.setText("请先取出药盘")
            self.ui.finish_dispensing_button.setEnabled(False)
            
            # 2秒后恢复按钮状态
            QTimer.singleShot(2000, self.restore_finish_button)

    def restore_finish_button(self):
        """恢复完成按钮状态"""
        if hasattr(self.ui, 'finish_dispensing_button'):
            self.ui.finish_dispensing_button.setText("完成分药")
            self.ui.finish_dispensing_button.setEnabled(True)

    def proceed_with_finish_dispensing(self):
        """继续执行完成分药流程"""
        # 关闭药盘
        from PySide6.QtCore import QMetaObject, Qt
    ...
    def safe_finish_dispensing(self):
        """安全完成分药流程"""
        if self.checking_plate_presence:
            return  # 正在检查中，忽略重复点击
        
        # 禁用按钮防止重复点击
        self.ui.finish_dispensing_button.setEnabled(False)
        self.ui.finish_dispensing_button.setText("检查中...")
        self.checking_plate_presence = True
