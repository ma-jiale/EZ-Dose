...
class PillCounter:
    ...
    def preprocess_image(self, frame):
        ...
        binary = cv2.erode(binary, erosion_kernel, iterations=3)

        cv2.imshow("2腐蚀后", binary)
        
        # # 膨胀操作：恢复轮廓大小，但保持分离效果
        dilation_kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (6, 6))
        binary = cv2.dilate(binary, dilation_kernel, iterations=3)

        cv2.imshow("3.膨胀后", binary)
