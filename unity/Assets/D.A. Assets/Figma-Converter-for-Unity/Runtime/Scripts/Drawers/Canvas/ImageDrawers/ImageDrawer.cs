...
namespace DA_Assets.FCU.Drawers.CanvasDrawers
{
    ...
    public class ImageDrawer : FcuBase
    {
        public override void Init(FigmaConverterUnity monoBeh)
        {
            ...
            UnityImageDrawer.Init(monoBeh);
            SpriteRendererDrawer.Init(monoBeh);

#if SUBC_SHAPES_EXISTS
            Shapes2DDrawer.Init(monoBeh);
#endif

#if JOSH_PUI_EXISTS
            JoshPuiDrawer.Init(monoBeh);
#endif

#if PROCEDURAL_UI_ASSET_STORE_RELEASE
            DttPuiDrawer.Init(monoBeh);
#endif

#if MPUIKIT_EXISTS
            MPUIKitDrawer.Init(monoBeh);
#endif

#if VECTOR_GRAPHICS_EXISTS
            SvgImageDrawer.Init(monoBeh);
#endif

#if NOVA_UI_EXISTS
            NovaImageDrawer.Init(monoBeh);
#endif
        }
